...
def compress_epub(epub_file, quality, output_dir):
    print(f"\n{Fore.YELLOW}Inizio compressione: {epub_file}")  # Inizio compressione
    ...
    # Comprime le immagini con una barra di progresso
    with tqdm(total=len(image_paths), desc=f"Comprimendo immagini in {epub_file}", unit="immagine") as pbar:
    ...
    print(f"{Fore.GREEN}Fine compressione con successo: {epub_file}")  # Fine compressione
